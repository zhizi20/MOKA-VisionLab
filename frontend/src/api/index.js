...
export const datasetApi = {
  ...
  importZip: (form) => request.post('/datasets/import-zip', form, { timeout: 0 }),
  startExport: (id, data) => request.post(`/datasets/${id}/export`, data, { timeout: 0 }),
  exportProgress: (jobId) => request.get(`/datasets/export-progress/${jobId}`, { timeout: 0 }),
  exportFile: (jobId, onDownloadProgress) => request.get(`/datasets/export-file/${jobId}`, {
    responseType: 'blob',
    timeout: 0,
    onDownloadProgress,
  }),
